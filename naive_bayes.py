...
def process_data(imgs: list[ImageData], p: BayesParamCombo):
    ...
    if (p.conv_size > 0): # 2D shortening
        ...
        for i in range(len(imgs)):
            new_arr = []
            # img_dim_size = sqrt(len(imgs[i].data))
...
def tune_hyperparams_full(truth_dict, train_images, val_images):
    """
    Offline hyperparameter tuning used when determining best model
    parameters before submitting.
    """
    param_combos: list[BayesParamCombo] = []
    for i in np.arange(0.01, 0.3, 0.04):  # LAP Number
        for j in range(2, 9, 1):  # Number of grayscale values
            for k in range(1, 9, 1): # 1D downscaling
                param_combos.append(BayesParamCombo(i, j, k, 0))
            for k in range(1, 3, 1):  # 2D downscaling
                param_combos.append(BayesParamCombo(i, j, 0, k))
...
def main():
    ...
    if mode == 1:
        ...
        best_params = tune_hyperparams_full(truth_dict, train_images, val_images)
        # Do final training and evaluate test data
        train_test(truth_dict, train_images, val_images, test_images, args.o, best_params)
    else:
        # Tune hyperparameters
        best_params = BayesParamCombo(1, 7, 1)
        # Do final training and evaluate test data
        train_test(truth_dict, train_images, val_images, test_images, args.o, best_params)
